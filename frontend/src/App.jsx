...
function App() {
  ...
  const [prueba2, setDataP2] = useState(null);
  const [data, setData] = useState(null);

  useEffect(() => {
    fetch(`https://backend-service-129342308367.us-central1.run.app/api/prueba`) // Nginx redirige al backend cuando esta configurado el proxy en el nginx.conf /api
      .then(res => res.json())
      .then(setDataP2)
      .catch(console.error);
